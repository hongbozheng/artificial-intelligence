...
def transformToMaze(alien, goals, walls, window, granularity):
    ...
    start_x = alien.get_config()[0]
    start_y = alien.get_config()[1]
    start_shape = alien.get_config()[2]
    width = int(window[0]/granularity)+1
    height = int(window[1]/granularity)+1
    maze_3D = np.full((width,height,3), SPACE_CHAR)
    digital_xy = configToIdx([start_x, start_y, start_shape], (0, 0, 0), granularity, alien)

    for i in range(width):
        for j in range(height):
            for k in range(3):
                pos = idxToConfig((i, j, k), (0, 0, 0), granularity, alien)
                if k == 0:
                    alien.set_alien_config([pos[0], pos[1], 'Horizontal'])
                if k == 1:
                    alien.set_alien_config([pos[0], pos[1], 'Ball'])
                if k == 2:
                    alien.set_alien_config([pos[0], pos[1], 'Vertical'])

                alien.set_alien_pos((pos[0], pos[1]))
                if i == digital_xy[0] and j == digital_xy[1] and k == digital_xy[2]:
                    maze_3D[i][j][k] = START_CHAR
                elif does_alien_touch_wall(alien, walls, granularity):
                    maze_3D[i][j][k] = WALL_CHAR
                elif not is_alien_within_window(alien, window, granularity):
                    maze_3D[i][j][k] = WALL_CHAR
                elif does_alien_touch_goal(alien, goals):
                    maze_3D[i][j][k] = OBJECTIVE_CHAR

    return Maze(maze_3D, alien, granularity)
